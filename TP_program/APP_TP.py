...
class Example(QMainWindow,Ui_MainWindow):
    ...
    def pdf_1(self):
        ...
        v=v1
        html,v,q,a,la = self.zadatak_2D_x(v,q,talj,la,a,z1,z2,y1,y2,y3,y4)
    ...
    def pdf_2(self):
        ...
        # html_max_t = self.Tzadatak_ciklus(d,U,eta,v,I,T0,T0_1,T0_2,T0_3,T0_4,y1,y2,y3,rez_x,rez_y,Thl1,Thl2,Thl3,Thl4)[0]
        # html_vrij_E = self.Tzadatak_vrijeme_E(d,U,eta,v,I,T0,T0_1,T0_2,T0_3,T0_4,y1,y2,y3,rez_x,rez_y,Thl1,Thl2,Thl3,Thl4)[0]
...
#        print(html)
    ...
    def Tzadatak_sirina_ZUT(self,d,U,eta,v,I,T0,T0_1,T0_2,T0_3,T0_4,y1,y2,y3,rez_x,rez_y,Thl1,Thl2,Thl3,Thl4,rho,cp,la):
        ...
        E = np.linspace(1, E, 1000, endpoint = True)
...
#        print(html)
    ...
    def Tzadatak_vrijeme_E(self,d,U,eta,v,I,T0,T0_1,T0_2,T0_3,T0_4,y1,y2,y3,rez_x,rez_y,Thl1,Thl2,Thl3,Thl4,rho,cp,la):
        ...
        # fig.add_trace(go.Scatter(x=E, y=t85_T1,
        #                     mode='lines',
        #                     name=f't85_T3={t85_T3}[m]'),
        #                       row=1, col=1)
        # fig.add_trace(go.Scatter(x=E, y=t85_T1,
        #                     mode='lines',
        #                     name=f't85_T4={t85_T4}[m]'),
        #                       row=1, col=1)
        
        fig.update_layout(height=650, width=800, title_text="t85"+"=f(E,T0) sa parametarskom vrijednošću T0")
...
#        print(html)
    ...
    def Tzadatak_ciklus(self,d,U,eta,v,I,T0,T0_1,T0_2,T0_3,T0_4,y1,y2,y3,rez_x,rez_y,Thl1,Thl2,Thl3,Thl4,rho,cp,la):
        ...
        # T03=200
        # T04=250
        #temp za proracun trenutne brzine hladenja
        # Th1=500
        # Th2=600
        # Th3=700
        # Th4=800
        ...
        tau = np.linspace(0.001, 150, 1000, endpoint = True)
...
#U+03C4
    ...
    def Dzadatak_2D_x(self,talj,d,q,roc,v,la,a,y1,y2,y3,y4):
        ...
        #fig = go.Figure()
        # fig = go.Figure(data=[go.Scatter3d(d3_5,
        #                                    mode='markers')])
    ...
    def zadatak_2D_x(self,v,q,talj,la,a,z1,z2,y1,y2,y3,y4):
        ...
        # d3_2=(x1,y1,z1_1)
        # d3_3 = np.hstack(( x1,y1,z1_1 )).ravel()
        ...
        d3_4 = np.dstack([x1,y1m,T1_1])
        d3_5 = dict(enumerate(d3_4.flatten(), 1))
        #talj=1600
        for i in range(0,len(T1_1),1):
            if (T1_1[i]>talj):
                T1_1[i]=talj
    ...
    def zadatak_2D_y(self,v,q,talj,la,a,z1,z2,x1,x2,x3,x4):
        ...
        # x1 = np.full((100), 0)        
        # x2 = np.full((100), 0.0015)
        # x3 = np.full((100), 0.0030)
        # x4 = np.full((100), 0.0045)
    ...
    def zadatak_2D_v(self,v,q,talj,la,a,z1,z2,v0,v1,v2):
        ...
        # y2 = np.full((100), 0.005)
        # y3 = np.full((100), 0.010)
        # y4 = np.full((100), 0.015)
        ...
        # r2_1 = np.sqrt( x1**2 + y2**2 + z1_1**2 )
        # r3_1 = np.sqrt( x1**2 + y3**2 + z1_1**2 )
        # r4_1 = np.sqrt( x1**2 + y4**2 + z1_1**2 )
        ...
        # r2_2 = np.sqrt( x1**2 + y2**2 + z1_2**2 )
        # r3_2 = np.sqrt( x1**2 + y3**2 + z1_2**2 )
        # r4_2 = np.sqrt( x1**2 + y4**2 + z1_2**2 )
        ...
        fig.update_layout(height=650, width=800, title_text=u"\u03d1"+"=f(x,v) sa parametarskom vrijednošću v")
        fig.update_layout(margin=dict(l=0, r=0, t=80, b=0))
        fig.update_xaxes(title_text="X [m]", row=1, col=1)
        fig.update_xaxes(title_text="X [m]", row=1, col=2)
        fig.update_yaxes(title_text="Temperatura [°C]", row=1, col=1)
    ...
    def zadatak_3D_z1(self, gornji_x, donji_x, gornji_y, donji_y,v,q,talj,la,a,z1,rez_x,rez_y):
        ...
        d_range=int(donji_y*rez_y)
        g_range=int((gornji_y)*rez_y+1)
        
        for i in range(d_range,g_range,1):
            cnt+=1
            n=np.full((rez_x), i/rez_y)
            y1_a.append(n)
        y1 = np.vstack(y1_a).ravel() 
        x1_0 = np.linspace(donji_x, gornji_x, rez_x, endpoint = True)
        x1 = np.tile(x1_0, cnt)
        z1_1 = np.full((cnt*rez_x), z1)   
        # z1_2 = np.full((100), 0.002)
        ...
        zaxis = dict(
            title='Temperatura [°C]'),),)
#        fig.update_zaxes(title_text="Temperatura [°C]")
        
        #fig.update_yaxis(title_text="Temperatura [°C]", row=1, col=1)
    ...
    def analiziram(self):
#        print(f"self.lineEdit.text:{self.lineEdit_12.text()}")
        i=0
        j=0
...
#            print(f"i:{i}")
        ...
        if (j==23):
            #problem je sto prolazi sve labele pa tako i 
            #onaj koji ima "" string pa normalno da ne 
            #moze pretvoriti "" u float 
            a,la,talj,q,gornji_x,donji_x,gornji_y,donji_y,z1,z2,y1,y2,y3,y4,x1,x2,x3,x4,v0,v1,v2,rez_x,rez_y = self.unos()

#            print(f"gornji_y:{gornji_y},{type(gornji_y)}")
#            print(f"donji_y:{donji_y},{type(donji_y)}")
            #self.pushButton_5.hide()
            #self.label.setText('Analiziram')
            print("Analiziram")
            v = v1
            ...
            html = self.zadatak_2D_x(v,q,talj,la,a,z1,z2,y1,y2,y3,y4)[0]
